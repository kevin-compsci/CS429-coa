...
#Description iaddq: 
# iaddq instruction is implemented to take advantage of
# known value and add the value to a destination. Iaddq takes a value and a 
# destination; as a result, the destination will hold the final result. This method
# is faster because we don't need to need two registers as inputs (one of them being 
# destination). Fetch must obtain a constant word and the instruction iaddq must be valid.
# Decode has to read the inputs (involving iaddq) for computation. Execution stage needs to
# put the known value into the destination and add if needed; the condition codes must also be set.
# The remaining stages can remain the same because iaddq functions similarly to the iopq instruction.
